...
def test_read_simple(dut):
    ...
    for sent in values:
        ...
        if val_string != str(sent):
            ...
            print("Corret value! Got : " + val_string + ", expected: " + str(sent))
            results.append("PASS")
        sleep(2)
        dut.board.reset()
...
def test_invalid_values(dut):
    ...
    for sent in commands:
        my_interface.write(sent)
        print("WRITE: " + (str(sent)).strip())
        value = read_value(my_interface)
        val_string = remove_whitespace(value)
        if val_string != remove_whitespace(sent):
            print("Incorrect value! Got : " + val_string + ", expected: " + str(sent))
            results.append("FAIL")
        else:
            print("Corret value! Got : " + val_string + ", expected: " + str(sent))
...
def test_measure(dut):
    ...
    for sent in range(0, 2000, 1):
        ...
        measurements.append(meas_vdc(0))
        
        if sent > 0:
            value_now = measurements[sent]
            value_before = measurements[sent-1]
            if(value_now > value_before):
                print("OK! Value is bigger than last value")
                results.append("PASS")
            else:
                print("FAIL! Value (" + str(measurements[sent]) + ")is not bigger than last value (" + str(measurements[sent-1]) +"(")
                results.append("FAIL")
        else:
            print("OK! First value is ok by default")
            results.append("PASS")
    # end test content
